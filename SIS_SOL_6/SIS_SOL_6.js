  ...
  if (modo==0) {   
   ...
   if ((millis()>(tiempo+gap)) && modo==1) { // permite controlar el tiempo de la animación
       ...
       if (instrucciones==1) {
         ...
         text("Cursores izquierda y derecha para acercar y alejar órbitas",1000,25);
         text("Tamaño planetas: un compromiso entre su tamaño real y la correcta visualización ",1000,105);
       }
...
class planetas {
  ...
  representar(coordX,coordY) { // método que dibuja en pantalla el planeta a su tamaño.
    ...
    this.planetSize = int(tp(this.diametro,30+log(fc)/log(1.2))); // con diametro de tierra
    if (this.nombre=="Sol") {this.planetSize=tpSun(30+log(fc)/log(1.2));}
  ...
  v() {
    ...
    let unx=tx(this.xxf(this.a,this.ex, this.E+0.01))-tx(this.xx); // aproximación vector normal
    let uny=ty(this.yyf(this.a,this.ex, this.E+0.01))-ty(this.yy);
    let ux = unx/sqrt(sq(unx)+sq(uny)); // aproximación vector normal normalizado
    let uy = uny/sqrt(sq(unx)+sq(uny)); 
...
function tp(diametro, zoom) {
 ...
 let maxM= 20; // 15 - 25 
 let A = (minJ-maxJ)/10;
 let B = (minJ-maxM-30*A)/(142990-4880);
 let C = maxM-10*A-B*4880;
 return int(A*zoom+B*diametro+C);  
 // let minimo = 50;
 // let maximo = 500;
 // let A = (minimo-maximo)/39;
 // let B = maximo-A;
 ...
 // let factor = 2;
 // let dNorm = -(factor-1)*diametro/(142990-4880)+(factor-4880*(-(factor-1)/(142990-4880)));
 // return A*zoom*dNorm+B; 
}

// funcion tamaño sol
function tpSun(zoom) {
  return int((-97/39)*zoom+(100+97/39));
}
